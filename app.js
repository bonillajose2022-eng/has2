...
const SECTIONS_NEEDED = 5; // sections to unlock final quiz
...
const libraryMatching = [
  { left: 'You can borrow', right: 'phone calls in the library.' },
  { left: 'You can\'t make', right: 'up to 5 books.' },
  { left: 'You can use', right: 'food or drinks.' },
  { left: 'You can\'t bring', right: 'notes quietly.' },
  { left: 'You can take', right: 'the computers for 1 hour.' },
  { left: 'You can\'t eat', right: 'photos for study.' },
  { left: 'You can listen', right: 'pets to the library.' },
  { left: 'You can\'t damage', right: 'to audiobooks with headphones.' }
];
...
/* ----- CLOTHES VOCABULARY ----- */
const clothesItems = [
  { emoji: '👔', word: 'shirt', hint: 'You wear this on your upper body.' },
  { emoji: '👗', word: 'dress', hint: 'Girls often wear this.' },
  { emoji: '👟', word: 'shoes', hint: 'You wear these on your feet.' },
  { emoji: '🎩', word: 'hat', hint: 'You wear this on your head.' },
  { emoji: '🧥', word: 'jacket', hint: 'You wear this over your clothes when cold.' },
  { emoji: '👖', word: 'trousers', hint: 'You wear these on your legs.' },
  { emoji: '🧦', word: 'socks', hint: 'You wear these inside your shoes.' },
  { emoji: '👘', word: 'skirt', hint: 'Girls wear this below the waist.' },
  { emoji: '🥾', word: 'boots', hint: 'Heavy shoes that cover your ankle.' },
  { emoji: '🧤', word: 'gloves', hint: 'You wear these on your hands in winter.' },
  { emoji: '🧣', word: 'scarf', hint: 'You wear this around your neck.' },
  { emoji: '🩲', word: 'coat', hint: 'A long warm outer garment.' }
];
...
function buildClothesExercises() {
  ...
  if (labelEl) {
    labelEl.innerHTML = clothesItems.map((c, i) => `
      <div class="clothes-label-item question-item" id="clothes-label-q${i}">
        <span class="clothes-emoji" role="img" aria-label="${c.word}">${c.emoji}</span>
        <span class="clothes-label-prompt">What is this? 👉</span>
        <input type="text" class="fill-input" id="cl-${i}" data-answer="${c.word}" placeholder="Type here..." aria-label="Type the name of this clothing item">
        <div class="explanation" id="cl-exp${i}">Answer: <strong>${c.word}</strong> — ${c.hint}</div>
      </div>
    `).join('');
...
function checkExercise(id) {
  ...
  switch(id) {
    ...
    case 'clothes-label': {
      const inputs = document.querySelectorAll('#clothes-label-exercises .fill-input');
      total = inputs.length;
      inputs.forEach((inp, i) => {
        const pass = inp.value.trim().toLowerCase() === inp.dataset.answer.toLowerCase();
        if (pass) correct++;
